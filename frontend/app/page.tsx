...
export default function Dashboard() {
  ...
  view cost analytics →
      ...
      <table className="w-full text-sm border-collapse">
        ...
        <tbody>
          {traces.map((t) => (
            <tr key={t.trace_id} className="border-b border-gray-800 hover:bg-gray-900">
              <td className="py-2 pr-4">
                <Link href={`/traces/${t.trace_id}`} className="font-mono text-blue-400 hover:underline">
                  {t.root_name}
                </Link>
              </td>
      ...
      {traces.length === 0 && <p className="text-gray-500">No traces yet — run your SDK tests.</p>}
    </main>
  );
}